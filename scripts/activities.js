...
const hideSign = document.getElementById("hideSign");
...
function displayActivityFeatures() {
  for (const activity of activities) {
    if (activity.name == activityList.selectedOptions[0].textContent) {
      ...
      descriptionParagraph.innerText = activity.description;
      locationParagraph.innerText = activity.location;
      ...
      if(buyTicketBtn.style.display == "block") {
...
function paymentDetails() {
    let numberTicket = Number(numberOfTicket.value);
    ...
    purchaseConfirmation.innerText = `Your credit card has been charged $${totalCost} for ${numberTicket} to
   ( ${seledtedActivity.innerText}). A confirmation email has been sent to ${email}`
